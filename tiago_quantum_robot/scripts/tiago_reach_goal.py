...
rospy.init_node('publish_reach_goal_with_voice')
human_present = False
publish_goal = False
...
reach_goal.joint_names = ['torso_lift_joint','arm_1_joint', 'arm_2_joint', 'arm_3_joint', 'arm_4_joint', 'arm_5_joint', 'arm_6_joint', 'arm_7_joint']
point = JointTrajectoryPoint()
point.positions = [0.31,1.43, -0.39, -2.97, 0.85, -2.07, 0.25, 1.70]
    ...
    if not human_present: # detect human only once
        ...
        for c in cnts:
            if cv2.contourArea(c) > 500:
                ...
                human_present = True
                soundhandle.say("Published reach goal message", voice, volume)
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
    else: # publish topic continuously
